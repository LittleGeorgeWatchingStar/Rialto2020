				...
				switch ($NAD[0]){
					...
					case 'DP':
						...
						if (strlen($NAD_C059[0])>0){
							$Order->DelAddr1 = $NAD_C059[0];
							$Order->DelAddr2 = $NAD_C059[1];
					...
					case 'SN':
						...
						$BranchResult = DB_query("SELECT BranchCode, BrName, BrAddr1,BrAddr2, BrMailStop, BrCity, BrState, BrZip, BrCountry, ContactName, DefaultLocation, PhoneNo, Email FROM CustBranch INNER JOIN DebtorsMaster ON CustBranch.DebtorNo = CustBranch.DebtorNo WHERE CustBranchCode='" . $NAD_C082[0] . "' AND CustBranch.DebtorNo='" . $Order->DebtorNo . "' AND DebtorsMaster.EDIOrders=1",$db);
						if (DB_num_rows($BranchResult)!=1){
							...
							$Order->Addr1 = $BranchRow['BrAddr1'];
							$Order->Addr2 = $BranchRow['BrAddr2'];
							$Order->MailStop = $BranchRow['BrMailStop'];
							$Order->City = $BranchRow['BrCity'];
							$Order->State = $BranchRow['BrState'];
							$Order->Zip = $BranchRow['BrZip'];
							$Order->Country = $BranchRow['BrCountry'];
