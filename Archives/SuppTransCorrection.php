...
if (isset($_POST['Post'])) {
	...
	$iterateSQL = "SELECT * FROM GLTrans WHERE Type=$TypeID AND TypeNo=$TransNo";
	$iterateResult = DB_query($iterateSQL, $db, $ErrMsg);
	while ( $rowI = DB_fetch_array( $iterateResult )) {
		$CounterIndex = $rowI['CounterIndex'];
		echo $CounterIndex;
	        if (isset($_POST['DR_Account_' . $CounterIndex])) {
			$Amount  =  $_POST['DR_Amount_' . $CounterIndex] ;
		        $Account =  $_POST['DR_Account_' . $CounterIndex];
		        $Ref = $_POST['DR_Ref_'. $CounterIndex ];
		        $SQL = "INSERT INTO GLTrans (Type, TypeNo, TranDate, Narrative, Amount, Account, PeriodNo ) VALUES
		                ($TypeID, $TransNo, '$TranDate', '$Ref', $Amount, $Account, $Prd )";
...
$SQL = "SELECT * FROM SuppTrans WHERE	SuppTrans.Type	= $TypeID AND SuppTrans.TransNo	= $TransNo";
...
echo "<BR> The amount will be changed by: " . number_format($_POST['y'],2) . "<BR><BR>";
	...
	if ($myrow['Account'] == '20000') {
		...
		Input_Hidden("CR_Ref",$myrow['Narrative']);
	       ...
	       printf ('<td>%s</td>
			...
			<td>%s</td>
			<td ALIGN=RIGHT>%s</td>
	                <td><INPUT TYPE=TEXT NAME="NewAmount_%s" SIZE=12 MAXLENGTH=12 Value="%s"></td>
			<td>%s</td>
			<td>%s</td>
			</tr>',
			$FormatedTranDate,
			...
			$myrow['AccountName'],
	                number_format($myrow['Amount'],2),
			$myrow['CounterIndex'],
			$_POST['NewAmount_' . $myrow['CounterIndex'] ],
			$myrow['Narrative'],
			$Posted);
	        if ( $_POST["NewAmount_" . $myrow['CounterIndex']] != 0 ) {
	                Input_Hidden("DR_Ref_" . $myrow['CounterIndex'] ,$myrow['Narrative']);
	                Input_Hidden("DR_Account_" .$myrow['CounterIndex'] ,$myrow['Account']);
	                Input_Hidden("DR_Amount_" .$myrow['CounterIndex'] , 
					$_POST["NewAmount_" . $myrow['CounterIndex']] - $myrow['Amount']);
        	        $sum += $_POST["NewAmount_" . $myrow['CounterIndex']] 
				- $myrow['Amount'];
		}
        }
        $j++;
