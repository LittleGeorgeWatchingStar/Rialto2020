...
while ( $this_card_tran = DB_fetch_array( $res ) ) {
	...
	$findSQL =  'SELECT * FROM BankTrans WHERE Type in (101,102) AND TransDate="' . $this_card_tran['PostDate'] . '" ' . $card_def_sql . ';';
	if ($bank_from_res = DB_fetch_array ( DB_query ( $findSQL, $db ) )) {
		...
		$findSQL='SELECT * FROM DebtorTrans WHERE Type=' . $this_card_tran['Type'] . ' AND TransNo = ' .  $this_card_tran['TransNo'];
		$debtortrans= DB_fetch_array ( DB_query ( $findSQL, $db ) );
	} else {
		echo '<BR>No joy on: ' . $findSQL;
	}

	$findSQL= 'SELECT DebtorTrans.* FROM CustAllocns 
		   LEFT JOIN DebtorTrans ON DebtorTrans.ID=CustAllocns.TransID_AllocTo WHERE TransID_AllocFrom  = '.$debtortrans['ID'];
	$invoicetrans= DB_fetch_array ( DB_query ( $findSQL, $db ) );
	...
	if (!check_to_bool( $_POST[ 'SELECT_' . $this_card_tran['CardTransID'] ] ) ) {
		...
		$cr = '<BR>';
		$failed = false;
		if (($_POST['NewOvAmount']!=$invoicetrans['OvAmount']) OR ($_POST['NewOvGST']!=$invoicetrans['OvGST']) OR ($_POST['NewOvFreight']!=$invoicetrans['OvFreight'])) {
			$ret = DB_query( 'BEGIN', $db);
	
			$FixSQL ='DELETE FROM CardTrans WHERE CardTransID= ' .  $_POST['SelectedCardTransID'] . ';';
