...
use Rialto\AccountingBundle\Entity\Currency;use Rialto\AccountingBundle\Entity\BankAccount;$PageSecurity = 5;
...
die("This page has been disabled.  Contact tech support.");
...
if ( isset($_POST['CommitBatch']) ) {
    ...
    /* once the GL analysis of the payment is entered (if the Creditors_GLLink is
     * active), process all the data in the session cookie into the DB creating a
     * banktrans record for the payment in the batch and SuppTrans record for the
     * supplier payment if a supplier was selected
     *
     * A GL entry is created for each GL entry (only one for a supplier entry)
     * and one for the bank account credit.
     *
     * NB allocations against supplier payments are a seperate exercice
     *
     * if GL integrated then first off run through the array of payment items
     * $_SESSION['Payment']->GLItems and create GL Entries for the GL payment items
     */
    ...
    if ( $_SESSION["PaymentDetail"]->SupplierID == "" ) {
        ...
        if ( $_SESSION['PaymentDetail']->PaymentType == 'Cheque' ) {
            if ( empty($_POST['ChequeNo']) ) {
                throw new Exception("No cheque number specified");
            }
            elseif (! is_numeric($_POST['ChequeNo']) ) {
                throw new Exception("Invalid cheque number");
            }
            $ChequeNo = (int) $_POST['ChequeNo'];
            $SuppReference = $ChequeNo;

            /* Increment cheque number */
            $SQL="UPDATE BankAccounts SET NextCheckNumber = NextCheckNumber + 1
                WHERE AccountCode = '" .
                $_SESSION['PaymentDetail']->Account . "'";
            DB_query($SQL, $db,'','',false,false);
            if ( DB_error_no($db)) {
                throw new Exception(DB_error_msg($db));
            }
        }
        else {
            $ChequeNo = 0;
            $SuppReference = $_SESSION['PaymentDetail']->PaymentType;
        }
...
$SQL = "SELECT BankAccountName,
        ...
        BankAccounts.NextCheckNumber
	FROM BankAccounts,
		ChartMaster
	WHERE BankAccounts.AccountCode=ChartMaster.AccountCode
    ORDER BY BankAccounts.AccountCode asc";
...
echo '<SELECT name="BankAccount" id="BankAccount">';
...
echo '<TR><TD>' . _('Payment Type') . ':</TD><TD>';
echo '<SELECT name="PaymentType" id="PaymentType">';
...
echo '<tr><td><label for="ChequeNo">Cheque Number:</label></td>';
echo '    <td><input type="number" id="ChequeNo" name="ChequeNo" /></td></tr>';
...
?>
<script type="text/javascript" src="/js/Accounting/ChequeNumberInput.js"></script>
<script type="text/javascript">
    dojo.addOnLoad(function() {
        new ChequeNumberInput(
            "BankAccount",
            "PaymentType",
            "ChequeNo"
        );
