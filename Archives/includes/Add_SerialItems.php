...
if ( isset($_POST['AddBatches']) && $_POST['AddBatches']!='') {
    ...
    for ($i=0;$i < 10;$i++){
        if ( (strlen($_POST['SerialNo' . $i])==0) && ($_POST['Qty' . $i]!=0) )  {
            $_POST['SerialNo' . $i] = GetNextSerialNo( $db );
        }
            ...
            if ($ItemMustExist){
                ...
                if ($ExistingBundleQty >0){
                    ...
                    if ($AddThisBundle==true){
                        $LineItem->SerialItems[$_POST['SerialNo' . $i]] =
                        new SerialItem($_POST['SerialNo' . $i],
    ...
    for ($i=0;$i < count($_POST['Bundles']);$i++){ /*there is an entry in the multi select list box */
        if ($LineItem->Serialised==1){	/*only if the item is serialised */
            $LineItem->SerialItems[$_POST['Bundles'][$i]] = new SerialItem ($_POST['Bundles'][$i],  ($InOutModifier>0?1:-1) , 'Code1' );
        } else {
            list($SerialNo, $Qty, $Version ) = explode ('/|/', $_POST['Bundles'][$i]);
            if ($Qty != 0) {
                $LineItem->SerialItems[$SerialNo] =
                new SerialItem ($SerialNo,  $Qty*($InOutModifier>0?1:-1), BinStyleOf( $SerialNo, $db ), $Version );
            }
        }
    }
    ...
    while (!feof($handle)) {
        ...
        if ($ItemMustExist){
            ...
            if ($ExistingBundleQty >0){
                ...
                if ($AddThisBundle==true){
                    $LineItem->SerialItems[$NewSerialNo] = new SerialItem ($NewSerialNo, $NewQty, 'Code2' );
                }
            } /*end if ExistingBundleQty >0 */
            else {
                echo '<BR>';
            ...
            $LineItem->SerialItems[$NewSerialNo] = new SerialItem ($NewSerialNo, $NewQty,'Code3');
        }
    }//while (file)
