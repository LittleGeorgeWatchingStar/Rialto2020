...
$CheckDate = Date($DefaultDateFormat);
...
if (isset($_POST['PrintPDFAndProcess'])){
	...
	$YPos -= $line_height;
	$LeftOvers = $pdf->addTextWrap($Left_Margin+10,$YPos,340-$Left_Margin,$FontSize,$_POST['BankAccount'].' '._('Check: ') . ' ' . $PaytReference, 'left');
	...
	/* Get the checknumber right... and increment the NextCheckNumber */
	$NextCheckNumber =  $_POST['Ref'] + $RefCounter;
	$SQL="UPDATE BankAccounts SET NextCheckNumber = " . $NextCheckNumber . " WHERE AccountCode = '" . $_POST['BankAccount'] . "'";
	$ProcessResult = DB_query($SQL,$db,'','',false,false);
	if (DB_error_no($db) !=0) {
		$title = _("Payment Processing") . " - " . _("Problem Report") . ".... ";
		include("includes/header.inc");
	...
	if ($CompanyRecord["GLLink_Creditors"]==1){
		...
		$PeriodNo = GetPeriod( $CheckDate ,$db);
