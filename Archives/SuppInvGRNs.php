...
if (isset($_POST['AddPOToTrans']) AND $_POST['AddPOToTrans']!=''){
    foreach($_SESSION['SuppTransTmp']->GRNs as $GRNTmp) {
        if ($_POST['AddPOToTrans']==$GRNTmp->OrderNo) {
...
if (DB_num_rows($GRNResults)==0){
	...
	echo "<P><A HREF='/index.php/record/Purchasing/PurchaseOrder/?completed=no&supplier=" . $_SESSION['SuppTrans']->SupplierID ."'>" . _('Select Purchase Orders to receive') .'</A>';
	include('includes/footer.inc');
	exit;
}
...
//	}
	...
	echo "<INPUT TYPE=HIDDEN NAME='GLCode' Value='" . $GRNTmp->GLCode . "'>";
        echo "<INPUT TYPE=HIDDEN NAME='OrderNo' Value='" . $GRNTmp->OrderNo. "'>";
    ...
    if (count( $_SESSION['SuppTransTmp']->GRNs)>0){   /*if there are any outstanding GRNs then */
        ...
        $POs = array();
        foreach ($_SESSION['SuppTransTmp']->GRNs as $GRNTmp){
            if ($POs[$GRNTmp->OrderNo] != $GRNTmp->OrderNo) {
                $POs[$GRNTmp->OrderNo] = $GRNTmp->OrderNo;
                echo "<TR><TD><INPUT TYPE=Submit Name='AddPOToTrans' Value='" . $GRNTmp->OrderNo. "'></TD><TD COLSPAN=3>" . _('Add Whole PO to Invoice') . '</TD></TR>';
                $i = 0;
            }
            if ($i == 0)
                echo $tableheader;
