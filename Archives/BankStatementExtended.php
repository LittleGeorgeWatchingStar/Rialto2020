...
<tr><td>Only Amex</td><td><INPUT type="radio" name="specials" value="OnlyAmEx"></td></tr>
<tr><td>Exclude Amex</td><td><INPUT type="radio" name="specials" value="NoAmEx"></td></tr>
<tr><td>Everything</td><td><INPUT type="radio" name="specials" value="None"></td></tr>
...
/*Get the balance of the bank account concerned */
	...
	$SQL =	" SELECT * FROM BankStatements WHERE Left(BankPostDate,7) LIKE '" . $_POST['TransMonth'] . "'";
	if ( $_POST['specials']=='OnlyAmEx' ) {
		$SQL .= " AND BankDescription LIKE '%EXPRESS%' ";
	}

	if ( $_POST['specials']=='NoAmEx' ) {
		$SQL .= " AND BankDescription NOT LIKE '%EXPRESS%' ";
	}
	$SQL .=	" ORDER BY " . 
		" SIGN(Amount), " . 
...
//		if (substr($lastType,0,3) != substr($myrow["BankDescription"],0,3) ) {
		if ( $last_date == '') {
			$last_date = $_POST['TransMonth'] . '01';
		}
		if ( $this_sign == ( $myrow["Amount"] > 0 ) ) {
			$this_sign = ! $this_sign; 
			echo $TableHeader;
		...
		if ( ($myrow["BankTransID"] != '0') && ($myrow["BankTransID"] != '') )  {
			$btsql	= "SELECT SUM(Amount) FROM BankTrans WHERE BankTransID IN (" . $myrow["BankTransID"] . ")";
			$btres	= DB_query($btsql, $db);
			if (DB_num_rows($btres)==1) {
				$btres  = DB_fetch_row($btres);
				$btsum	= $btres[0];
				$btlist = $myrow["BankTransID"];
			}
		} else {
			$this_date = $myrow["BankPostDate"];
			if ( $this_date != $last_date ) {
				$sub_sql = 'SELECT * FROM BankTrans WHERE TransDate >"' .$last_date . '" AND TransDate <="' . $this_date . '" AND AmountCleared=0 ';
				if ( $this_sign ) {
				...
				if ( $_POST['specials'] =='NoAmEx' ) {
					$sub_sql .= " AND Ref NOT LIKE '%AmEx%' ";
				}
                                if ( $_POST['specials'] =='OnlyAmEx' ) {
                                        $sub_sql .= " AND Ref LIKE '%AmEx%' ";
                                }
				$sub_ret = DB_query( $sub_sql, $db);
				while ( $bt_row = DB_fetch_array( $sub_ret) ) {
					echo	'<tr><td>' . $bt_row['TransDate'] . '</td><td><i>    ' . $bt_row['Ref'] . '</i></td>' . '<td colspan=2></td>' .  
						'<td>' . $bt_row['Amount'] . '</td>' . '</tr>';
				}
				$last_date = $this_date;
			}
			if ($k==1){
		...
		$YPos -=13;
	        ...
	        $LeftOvers = $pdf->addTextWrap( $Left_Margin+410,$YPos, 50,$FontSize,number_format($myrow["Amount"],2),'right');
                $LeftOvers = $pdf->addTextWrap( $Left_Margin+460,$YPos, 50,$FontSize,$myrow["BankTransID"]);
                $LeftOvers = $pdf->addTextWrap( $Left_Margin+510,$YPos, 50,$FontSize,number_format($myrow["BankTransID"]==''?'':$btres['Amount'],2),'right');
		if ($YPos<50)  {
			$PageNumber+=1;
			include ('includes/PDF_BankStatement.inc');
			$line_height=14;
