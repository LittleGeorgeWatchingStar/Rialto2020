...
include_once("includes/CommonGumstix.inc");
	...
	$line_height=16;
      ...
      $SQL = "SELECT StockMaster.CategoryID,
		     ...
		     INNER JOIN Locations   ON LocStock.LocCode=Locations.LocCode
                     WHERE StockMaster.MBflag='B'  AND StockMaster.Discontinued=0 AND  LocStock.LocCode IN (7,8,9)
	...
	While ($InventoryPlan = DB_fetch_array($InventoryResult,$db)){
		...
		$locOffset = min ( 80*($InventoryPlan['LocCode']-3), 484) ;
		...
		$inside_offset = 0;
			...
			$inside_offset+=$line_height-2;
                        $LeftOvers = $pdf->addTextWrap($locOffset,$YPos - $inside_offset,60,$FontSize-1,
							$id. '('  . number_format($qty,0) . ')', 'right');
                }
