...
elseif ( $_GET['InvoiceNumber'] > 0 ) {
    ...
    if ( DB_num_rows($GetInvHdrResult) == 1 ) {
        ...
        /* now populate the line items array with the stock
         * movement records for the invoice */
        ...
        if ( db_num_rows($LineItemsResult) > 0 ) {
            ...
            while ( $myrow = db_fetch_array($LineItemsResult) ) {
                ...
                $_SESSION['CreditItems']->add_to_cart($myrow["StockID"], $myrow["Quantity"], $myrow["Description"], $myrow["Price"], $myrow["DiscountPercent"], $myrow["DiscountAccount"], $myrow["Units"], $myrow["Volume"], $myrow["KGS"], 0, // QOHAtLoc
                    $myrow["MBflag"], $myrow['TranDate'], 0, // QtyInvoiced
                    $myrow['DiscountCategory'], $myrow['Controlled'], $myrow['Serialised'], $myrow['DecimalPlaces'], $myrow['Narrative'], '', // Custom
                    '', // Customization
                    'No'	// UpdateBD
                );
    ...
    foreach ( $_SESSION['CreditItems']->LineItems as $StockItem ) {
        ...
        if ( isset($_POST['Quantity_' . $StockItem->StockID]) ) {
            ...
            If ( $Quantity < 0 OR $Price < 0 OR $DiscountPercentage > 100 OR $DiscountPercentage < 0 ) {
                ...
                $_SESSION['CreditItems']->LineItems[$StockItem->StockID]->DiscountAccount = $DiscountAccount;
...
foreach ( $_SESSION['CreditItems']->LineItems as $LnItm ) {
    ...
    echo '<TD><SELECT NAME="Discount_Account_' . $LnItm->StockID . '">';
    while ( $myrow = DB_fetch_array($result) ) {
        echo '<OPTION VALUE=' . $myrow['AccountCode'] . ($myrow['AccountCode'] == $LnItm->DiscountAccount ? ' SELECTED' : '') . '>' . _($myrow['AccountName']) . '</OPTION>';
    }
    echo '</SELECT></TD>';
    echo "<TD ALIGN=RIGHT>$DisplayLineTotal</TD><TD ALIGN=RIGHT>" . round($StockItem->TaxRate * 100, 4) . "%</TD>
...
if ( isset($_POST['ProcessCredit']) ) {
    ...
    if ( $myrow[0] > 0 ) { /* the invoice is not already fully allocated */
        ...
        if ( $myrow[0] > ($_SESSION['CreditItems']->total + $_POST['ChargeFreightCost'] + $TaxTotal) ) {
            ...
            /* the balance left to allocate is less than
             * the credit note value */
            $Allocate_amount = $myrow[0]; // unallocated balance from invoice
            $SettledInvoice = 1;
            $Settled = 0;
        }

        /* Now need to update the invoice DebtorTrans record for the amount
         * to be allocated and if the invoice is now settled */
    ...
    /* Now insert the Credit Note into the DebtorTrans table with
     * the allocations as calculated above */
    ...
    /* Update sales order details quantity invoiced less this credit
     * quantity. */
    ...
    foreach ( $_SESSION['CreditItems']->LineItems as $OrderLine ) {
        ...
        if ( $OrderLine->QtyDispatched > 0 ) {
            ...
            $Result = DB_query($SQL, $db, _('Could not determine if the item') . ' ' . $OrderLine->StockID . ' ' . _('is purchased or manufactured'), _('The SQL used that failed was'), true);
            ...
            if ( $_POST['CreditType'] == "Return" ) {
                ...
                /* Insert stock movements for the stock coming back in -
                 * with unit cost */
