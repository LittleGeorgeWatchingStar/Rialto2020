...
use Rialto\AccountingBundle\Entity\GLEntry;
...
if ( isset($_POST['CommitBatch']) ) {
    ...
    foreach ( $_SESSION['ReceiptBatch']->Items as $ReceiptItem ) {
        ...
        if ( $ReceiptItem->GLCode != "" ) {
            ...
            /* check to see if this is a GL posting to another bank account
             * (or the same one)
    ...
    if ( $CompanyRecord["GLLink_Debtors"] == 1 ) { /* then enter GLTrans records for discount, bank and debtors */
        ...
        if ( check_to_bool($_POST['PrepaymentFlag']) == 1 ) {
            $DebtorsAccount = 22000;
        }
        else {
            $DebtorsAccount = $CompanyRecord["DebtorsAct"];
        }
...
if ( ((isset($_SESSION['CustomerRecord']) AND isset($_POST['CustomerID']) AND $_POST['CustomerID'] != "") OR isset($_POST['GLEntry'])) AND isset($_SESSION['ReceiptBatch']) ) {
    ...
    echo '<TR><TD>' . _('Amount of Receipt') . ":</TD><TD><INPUT TYPE='text' name='amount' maxlength=12 size=13 value='" . $_POST['amount'] . "'>";
