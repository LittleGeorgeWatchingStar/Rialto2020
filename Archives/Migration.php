...
function X_getQOHOnDate($StockID, $db, $DateTo, $DateFrom)
{
	if ($DateTo != null) {
                $SQLDateTo = FormatDateForSQL($DateTo);
		$SQLDateFrom= FormatDateForSQL($DateFrom);
		$sql = "SELECT SUM(Qty) FROM StockMoves WHERE StockID='" . $StockID . "' AND TranDate >= '" . $SQLDateFrom . "' AND TranDate <= '" . $SQLDateTo . "'";
		$result = DB_query($sql, $db, "QOH calulcation failed");
...
If (isset($_POST['ShowToDate']) && !isset($_POST['PrintPDF'])) {
	...
	While ($InventoryValn = DB_fetch_array($InventoryResult,$db)){
		...
		if (( $qoh =  X_getQOHOnDate($InventoryValn['StockID'], $db, $_POST['ShowToDate'], $_POST['ShowFromDate']))  != 0) {
			if (!isset($currentpart)) {
				$currentpart = $InventoryValn['StockAct'];
				$runningtotal = $qoh * $InventoryValn['UnitCost'];
			}
	...
	echo "<INPUT TYPE='text' Name='ShowToDate' Value='".$_POST['ShowToDate']. "'>";
        echo "<INPUT TYPE='text' Name='ShowFromDate' Value='".$_POST['ShowFromDate']. "'>";
	...
	While ($InventoryValn = DB_fetch_array($InventoryResult,$db)){
		...
		if ($Category!=$InventoryValn['CategoryID']){
			...
			$CategoryName = $InventoryValn['CategoryDescription'];
                ...
                $QtyOnHand = X_getQOHOnDate($InventoryValn['StockID'],$db, $_POST['ShowToDate']);
...
/*Print out the grand totals */
	...
	/*if $FromCriteria is not set then show a form to allow input	*/
		...
		echo "<INPUT TYPE='text' Name='ShowToDate' Value='" .Date("m/d/Y")    . "'></CENTER>";
                echo "<INPUT TYPE='text' Name='ShowFromDate' Value='" .Date("m/d/Y")    . "'></CENTER>";
		}
	include('includes/footer.inc');
