...
use Rialto\AccountingBundle\Entity\Currency;
...
function FindPayment($AllocTransTo, $db)
{
    $SQL = " select STF.* from SuppAllocs
                inner join SuppTrans STF ON STF.ID=TransID_AllocFrom
                inner join SuppTrans STT ON STT.ID=TransID_AllocTo
                where TransID_AllocTo ='" . $AllocTransTo . "'";
    $Result = DB_query($SQL, $db);
    if ( DB_num_rows($Result) == 0 ) {
        return '';
    }
    $myrow = DB_fetch_array($Result);
    return $myrow['SuppReference'];
}
...
echo '<CENTER><FONT SIZE=4>' . $SupplierRecord['SuppName'] . ' </FONT></B> - (' . _('All amounts stated in') .
 ' ' . $SupplierRecord['Currency'] . ')</CENTER><BR><B><FONT COLOR=BLUE>' . _('Terms') . ': ' . $SupplierRecord['Terms'] . '</font></b>';
...
$TableHeader = "<TR><TD CLASS='tableheader'>" . _('Trans') . ' #' .
    ...
    "</TD><TD CLASS='tableheader'>" . _('Invoice  or Cheque') . ' #' .
    ...
    "</TD><TD CLASS='tableheader'>" . _('Comments') .
    "</TD><TD CLASS='tableheader'>" . _('Action') .
    "</TD><TD CLASS='tableheader'>" . _('Allocations') .
    "</TD><TD CLASS='tableheader'>" . _('GL Listings') . '</TD></TR>';
        ...
        if ( $CompanyRecord['GLLink_Creditors'] == True ) {
            If ( $myrow['TotalAmount'] - $myrow['Allocated'] == 0 ) {
                ...
                printf("<TD>%s</TD>
                    ...
                    number_format($myrow['TotalAmount'] - $myrow['Allocated'], 2),
                    $myrow['TransText'],
                    $_SERVER['PHP_SELF'], SID, $myrow['Type'], $myrow['TransNo'], $HoldValue, $_POST['TransAfterDate'], $HoldValue,
...
//                                        <TD> </TD>
//					<TD ALIGN=LEFT>%s</TD>
//					</TD></TR>", $myrow['TransNo'], $myrow['TypeName'], $myrow['SuppReference'], ConvertSQLDate($myrow['TranDate']), number_format($myrow['TotalAmount'], 2), number_format($myrow['Allocated'], 2), number_format($myrow['TotalAmount'] - $myrow['Allocated'], 2), $myrow['TransText']);
//            }
//            else {
//
//                printf("<TD>%s</TD>
...
//        }
        ...
        if ( $CompanyRecord['GLLink_Creditors'] == True ) {
            ...
            printf("<TD>%s</TD>
				...
				</TR>', $myrow['TransNo'], $myrow['TypeName'], $myrow['SuppReference'], ConvertSQLDate($myrow['TranDate']), number_format($myrow['TotalAmount'], 2), number_format($myrow['Allocated'], 2), number_format($myrow['TotalAmount'] - $myrow['Allocated'], 2), $myrow['TransText'], $rootpath, SID, $myrow['ID']);
        }
    }
