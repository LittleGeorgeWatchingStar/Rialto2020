...
die("This page has been disabled. Contact the system administrator.");

/* $Revision: 1.8 $ */
use Rialto\TaxBundle\Entity\TaxAuthority;
use Rialto\PurchasingBundle\Entity\Supplier;
use Rialto\AccountingBundle\Entity\Currency;use Rialto\AccountingBundle\Entity\PaymentTerms;/*The supplier transaction uses the SuppTrans class to hold the information about the invoice
...
/*Now retrieve supplier information - name, currency, default ex rate, terms, tax rate etc */
	...
	if ($myrow['DaysBeforeDue'] == 0){
		 $_SESSION['SuppTrans']->Terms = -$myrow['DayInFollowingMonth'];
	} else {
		 $_SESSION['SuppTrans']->Terms =  $myrow['DaysBeforeDue'];
	}
...
if (isset($_POST['ExRate'])){
	...
	if (substr( $_SESSION['SuppTrans']->Terms,0,1)=="1") { /*Its a day in the following month when due */
		...
		$InvoiceDateArray = explode("/",$_POST['TranDate']);
		$_SESSION['SuppTrans']->DueDate = Date($DefaultDateFormat, Mktime(0,0,0,$InvoiceDateArray[0],$InvoiceDateArray[1]+ $_SESSION['SuppTrans']->Terms,$InvoiceDateArray[2]) );
	}
...
then do the updates and inserts to process the invoice entered */
				...
				/* enter the GL entry to reverse the GRN suspense entry created on delivery at standard cost used on delivery */
					...
					if ($PurchPriceVar !=0){ /* don't bother with this lot if there is no difference ! */
						if (strlen($EnteredGRN->ItemCode)>0 OR $EnteredGRN->ItemCode != ''){ /*so it is a stock item */
							...
							$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR 1  AND SEEK ASSISTANCE') . ': ' . _('The general ledger transaction could not be added for the price variance of the stock item because');
							$DbgMsg = _('The following SQL to insert the GL transaction was used');
						...
						the goods were received */
							...
							$SQL = 'INSERT INTO GLTrans (Type, TypeNo, TranDate, PeriodNo, Account, Narrative, Amount)
								VALUES (20, ' .
									 ...
									 $EnteredGRN->GLCode . ", '" .
									 $_SESSION['SuppTrans']->SupplierID . ' - ' . _('GRN') . ' ' . $EnteredGRN->GRNNo . ' - ' .
									 	$EnteredGRN->ItemDescription . ' x ' .
										$EnteredGRN->This_QuantityInv . ' x  ' . _('price var') . ' ' .
										number_format(($EnteredGRN->ChgPrice  / $_SESSION['SuppTrans']->ExRate)
											- $EnteredGRN->StdCostUnit,2) .
									 "', " . $PurchPriceVar . ')';

							$ErrMsg = _('CRITICAL ERROR') . '! ' . _('NOTE DOWN THIS ERROR 2 AND SEEK ASSISTANCE') . ': ' . _('The general ledger transaction could not be added for the price variance of the stock item because');
	...
	/* TRYING THE STD COST UPDATE HERE... */
