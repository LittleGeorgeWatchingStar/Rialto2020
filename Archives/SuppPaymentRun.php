...
/*then print the report */
  ...
  /*Now figure out the invoice less credits due for the Supplier range under review */
	...
	if (isset($_POST['PrintPDFAndProcess'])){
		$ProcessRFesult = DB_query('begin',$db);
	}
	...
	while ($SuppliersToPay = DB_fetch_array($SuppliersResult)){
		...
		$sql = "SELECT Suppliers.SupplierID,
			...
			AND ROUND(SuppTrans.OvAmount + SuppTrans.OvGST - SuppTrans.Alloc) != 0
			AND SuppTrans.DueDate <='" . FormatDateForSQL($_POST['AmountsDueBy']) . "'
			AND SuppTrans.Hold=0
			AND Suppliers.CurrCode = '" . $_POST['Currency'] . "'
			AND SuppTrans.SupplierNo >= '" . $_POST['FromCriteria'] . "'
		...
		while ($DetailTrans = DB_fetch_array($TransResult)){
			...
			$DiffOnExch = ($DetailTrans['Balance'] / $DetailTrans['Rate']) -
                ($DetailTrans['Balance'] / $_POST['ExRate']);
			...
			if (isset($_POST['PrintPDFAndProcess'])){
				...
				/* Record the Allocations for later insertion once we have
                 * the ID of the payment SuppTrans */
				$Allocs[$AllocCounter] = new Allocation($DetailTrans['ID'],$DetailTrans['Balance']);
				$AllocCounter++;

				/* Now update the SuppTrans for the allocation made and the
                 * fact that it is now settled */
	...
	$SQL = 'SELECT BankAccountName, AccountCode, NextCheckNumber FROM BankAccounts';
	...
	if (DB_num_rows($AccountsResults)==0){
		...
		while ($myrow=DB_fetch_array($AccountsResults)){
			$_POST['StartingCheck_'.$myrow['AccountCode']] = $myrow['NextCheckNumber'];
...
Payment types can be modified by editing that file */
	...
	if (!is_numeric($_POST['Ref'])){
		$DefaultRef = $_POST['StartingCheck_'.$_POST['BankAccount']];
	} else {
		$DefaultRef = $_POST['Ref'];
	}
	echo '<TR><TD>' . _('Starting Check Number') . ':</TD>
            <TD><INPUT TYPE=text name="Ref" MAXLENGTH=11 SIZE=12 VALUE=' . $_POST['StartingCheck_'.$_POST['BankAccount']] . '> </TD></TR>';

	echo '</TABLE>
		<INPUT TYPE=Submit Name="CheckNumberData" Value="' . _('Get next checknumber') . '">
