...
if (isset($_POST['UpdateDatabase']) OR isset($_POST['RefreshAllocTotal'])) {
    ...
    for ($AllocCounter = 0; $AllocCounter < $_POST['TotalNumberOfAllocs']; $AllocCounter++) {
        ...
        if (isset($_POST['Amt' . $AllocCounter])) { // only do the below for allocatable charge amounts
            ...
            /* recalculate the new difference on exchange
...
if (isset($_POST['UpdateDatabase'])) {
    ...
    If ($InputError == 0) { /* ie all the traps were passed */
            ...
            if ($AllocnItem->OrigAlloc != $AllocnItem->AllocAmt) {
                ...
                if ($AllocnItem->AllocAmt > 0) {
                    $SQL = "INSERT INTO CustAllocns (DateAlloc,
                        ...
                        " . $AllocnItem->AllocAmt . ',
                        ' . $_SESSION['Alloc']->AllocTrans /* from the receipt */ . ',
                        ' . $AllocnItem->ID /* to the invoice */ . ')';

                    $ErrMsg = _('CRITICAL ERROR') . '! ' .
                        _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' .
                        _('The customer allocation record for') . ' ' .
                        $AllocnItem->TransType . ' ' . $AllocnItem->TypeNo .
                ...
                $SQL = 'UPDATE DebtorTrans SET DiffOnExch=' .
        ...
        $ErrMsg = _('CRITICAL ERROR') . '! ' .
            _('NOTE DOWN THIS ERROR AND SEEK ASSISTANCE') . ': ' .
            _('The debtor receipt or credit note transaction could not be modified for the new allocation and exchange difference because');
        $DbgMsg = _('The following SQL to delete the allocation record was used');
        $Result = DB_query($SQL, $db, $ErrMsg, $DbgMsg, true);
...
elseif (isset($_GET['DebtorNo'])) {
    ...
    while ($myrow = DB_fetch_array($result)) {
		...
		</tr>", $myrow['TypeName'], $myrow['Name'], $myrow['TransNo'], ConvertSQLDate($myrow['TranDate']), $myrow['Total'], $myrow['Total'] - $myrow['Alloc'], $_SERVER['PHP_SELF'] . '?' . SID, $myrow['ID'], _('Allocate'));
    }
}
