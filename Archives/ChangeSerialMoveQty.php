...
if (isset($_POST['Change'])) {
	...
	echo '</table>';
	echo '</FORM>';
	$sql = 'UPDATE StockSerialMoves SET MoveQty=' . $_POST['NewQty'] . " WHERE StkItmMoveNo=$StkItmMoveNo AND SerialNo=" . $row['SerialNo'];
        echo $sql . '<BR>';
	$res = DB_query($sql, $db );
	$sql = 'UPDATE StockSerialItems SET Quantity=Quantity+('. ($_POST['NewQty'] - $row['MoveQty']) .') WHERE SerialNo='. $row['SerialNo'].' AND LocCode='. $row['LocCode'];
        echo $sql . '<BR>';
        $res = DB_query($sql, $db );
